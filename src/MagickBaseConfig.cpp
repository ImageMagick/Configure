...
void MagickBaseConfig::write(const ConfigureOptions &options)
{
  wifstream configIn(options.rootDirectory + L"Projects\\MagickCore\\magick-baseconfig.h.in");
  if (!configIn)
  ...
  while (getline(configIn,line))
  {
    ...
    for (const auto& entry : filesystem::directory_iterator(options.rootDirectory + L"Artifacts\\config"))
    {
      ...
      wstring fileName=entry.path().wstring() + L"\\config.h";
