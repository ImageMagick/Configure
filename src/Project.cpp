...
Project::Project(const Config &config,const ConfigureOptions &options)
  : _config(config),
    _options(options)
{  
}
...
const wstring Project::configurationType() const
{
  if (_config.type() == ProjectType::Application ||
      _config.type() == ProjectType::Demo ||
      _config.type() == ProjectType::Fuzz)
...
const bool Project::isStatic() const
{
  if (_config.type() == ProjectType::Application ||
      _config.type() == ProjectType::Demo)
    return(false);

  if(_options.buildType == BuildType::Static)
    return(true);

  return(_config.type() == ProjectType::StaticLibrary);
}
...
const wstring Project::outputDirectory() const
{
  switch(_config.type())
  {
    ...
    case ProjectType::Coder: return(L"bin");
...
const wstring Project::prefix() const
{
  ...
  case ProjectType::Coder: return(L"IM_MOD");
...
vector<Project> Project::splitToFiles(const vector<wstring> additionalFiles) const
{
  ...
  for (const auto& file : _files)
  {
    if (!endsWith(file,L".c") && !endsWith(file,L".cc"))
      continue;
...
void Project::writeConfiguration(wofstream &file) const
{
  ...
  file << "    <ProjectConfiguration Include=\"Debug|" << _options.architectureName() << "\">" << endl;
  file << "      <Configuration>Debug</Configuration>" << endl;
  file << "      <Platform>" << _options.platform() << "</Platform>" << endl;
  file << "    </ProjectConfiguration>" << endl;
  file << "    <ProjectConfiguration Include=\"Release|" << _options.architectureName() << "\">" << endl;
...
void Project::writeLibProperties(wofstream& file) const
{
  ...
  file << "      <AdditionalOptions>/WX %(AdditionalOptions)</AdditionalOptions>" << endl;
  file << "    </Lib>" << endl;
  file << "  </ItemDefinitionGroup>" << endl;
}
...
void Project::writeMagickBaseconfigDefine() const
{
  ...
  wstring targetDirectory=_options.rootDirectory + L"Artifacts\\config\\" + name() + L"\\";
  filesystem::create_directories(targetDirectory);

  wstring configFileName=targetDirectory + L"config.h";
...
void Project::writeLinkProperties(wofstream& file) const
{
  ...
  file << "      <AdditionalDependencies Condition=\"'$(Configuration)'=='Release'\">" << additionalDependencies(false) << "%(AdditionalDependencies)</AdditionalDependencies>" << endl;
  file << "      <AdditionalOptions>/WX %(AdditionalOptions)</AdditionalOptions>" << endl;
...
void Project::writeReferences(wofstream &file,const vector<Project> &allProjects) const
{
  ...
  for (const auto& reference : _config.references())
  {
    auto project = find_if(allProjects.begin(), allProjects.end(),[&reference](const Project& p) { return(p.name() == reference); });
  ...
  if (type() == ProjectType::Application || type() == ProjectType::Fuzz)
  {
