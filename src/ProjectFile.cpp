...
//
//ProjectFile::ProjectFile(const std::wstring &name)
//{
//  _name=name;
//}
...
//  file << "    <ProjectConfiguration Include=\"Debug|" << _wizard->platformName() << "\">" << std::endl;
//  file << "      <Configuration>Debug</Configuration>" << std::endl;
//  file << "      <Platform>" << _wizard->platformName() << "</Platform>" << std::endl;
//  file << "    </ProjectConfiguration>" << std::endl;
//  file << "    <ProjectConfiguration Include=\"Release|" << _wizard->platformName() << "\">" << std::endl;
//  file << "      <Configuration>Release</Configuration>" << std::endl;
//  file << "      <Platform>" << _wizard->platformName() << "</Platform>" << std::endl;
//  file << "    </ProjectConfiguration>" << std::endl;
//  file << "  </ItemGroup>" << std::endl;
//  file << "  <PropertyGroup Label=\"Globals\">" << std::endl;
//  file << "    <ProjectName>" << _prefix << "_" << _name << "</ProjectName>" << std::endl;
//  file << "    <ProjectGuid>{" << _guid << "}</ProjectGuid>" << std::endl;
//  file << "    <Keyword>" << _wizard->platformName() << "Proj</Keyword>" << std::endl;
...
//    file << "    <PlatformToolset>v141</PlatformToolset>" << std::endl;
...
//  file << "    <OutDir>" << outputDirectory() << "</OutDir>" << std::endl;
//  if (_project->isExe())
//  {
//    file << "    <TargetName>" << _name << "</TargetName>" << std::endl;
//  }
//  else
//  {
//    file << "    <TargetName Condition=\"'$(Configuration)|$(Platform)'=='Debug|" << _wizard->platformName() << "'\">" << getTargetName(true) << "</TargetName>" << std::endl;
//    file << "    <TargetName Condition=\"'$(Configuration)|$(Platform)'=='Release|" << _wizard->platformName() << "'\">" << getTargetName(false) << "</TargetName>" << std::endl;
//  }
//  file << "    <IntDir Condition=\"'$(Configuration)|$(Platform)'=='Debug|" << _wizard->platformName() << "'\">" << getIntermediateDirectoryName(true) << "</IntDir>" << std::endl;
//  file << "    <IntDir Condition=\"'$(Configuration)|$(Platform)'=='Release|" << _wizard->platformName() << "'\">" << getIntermediateDirectoryName(false) << "</IntDir>" << std::endl;
//  if (_wizard->visualStudioVersion() >= VisualStudioVersion::VS2019)
//    file << "    <UseDebugLibraries Condition=\"'$(Configuration)|$(Platform)'=='Debug|" << _wizard->platformName() << "'\">true</UseDebugLibraries>" << std::endl;
//  file << "  </PropertyGroup>" << std::endl;
//
...
//  {
//    if (endsWith(f,L".rc"))
//      file << "    <ResourceCompile Include=\"" << f << "\" />" << std::endl;
//    else if (endsWith(f,L".h"))
//      file << "    <ClInclude Include=\"" << f << "\" />" << std::endl;
//    else if (endsWith(f,L".asm"))
//    {
//      if (!_project->useNasm())
//      {
//        file << "    <CustomBuild Include=\"" << f << "\">" << std::endl;
//        file << "      <Command>" << asmOptions() << "</Command>" << std::endl;
...
//        file << "    <CustomBuild Include=\"" << f << "\">" << std::endl;
//        file << "      <Command>" << nasmOptions(folder) << "</Command>" << std::endl;
//        file << "      <Outputs>$(IntDir)%(Filename).obj;%(Outputs)</Outputs>" << std::endl;
//        file << "    </CustomBuild>" << std::endl;
//      }
//    }
...
//
//      file << "    <ClCompile Include=\"" << f << "\">" << std::endl;
//      name=replace(f,L"..\\",L"");
//      if (contains(_cppFiles,name))
//        file << "      <CompileAs>CompileAsCpp</CompileAs>" << std::endl;
//      else if (count > 1)
//      {
//        name=f.substr(0,f.find_last_of(L"."));
//        name=name.substr(name.find_last_of(L"\\") + 1);
//        file << "      <ObjectFileName>$(IntDir)" << name << "_" << count << ".obj</ObjectFileName>" << std::endl;
//      }
...
//    if (filter != L"")
//    {
//      file << "    <" << tagName << L" Include=\"" << f << "\">" << std::endl;
//      file << "      <Filter>" << filter << "</Filter>" << std::endl;
//      file << "    </" << tagName << L">" << std::endl;
//    }
//    else
//      file << "    <" << tagName << L" Include=\"" << f << "\" />" << std::endl;
//  }
...
//  for (auto& f : filters)
//  {
//    file << "    <Filter Include=\"" << f << "\">" << std::endl;
//    file << "      <UniqueIdentifier>{" << _guid << "}</UniqueIdentifier>" << std::endl;
...
//
//  file << "  <ItemDefinitionGroup Condition=\"'$(Configuration)|$(Platform)'=='" << (debug ? "Debug" : "Release") << "|" << _wizard->platformName() << "'\">" << std::endl;
//  file << "    <ClCompile>" << std::endl;
//  file << "      <RuntimeLibrary>MultiThreaded" << (debug ? "Debug" : "") << (_wizard->solutionType() == SolutionType::STATIC_MT ? "" : "DLL") << "</RuntimeLibrary>" << std::endl;
...
//  else
//    file << "      <WarningLevel>Level" << _project->warningLevel() << "</WarningLevel>" << std::endl;
//  if (_project->treatWarningAsError())
//    file << "      <TreatWarningAsError>true</TreatWarningAsError>" << std::endl;
//  file << "      <SuppressStartupBanner>true</SuppressStartupBanner>" << std::endl;
//  if (_project->compiler() == Compiler::CPP)
//    file << "      <CompileAs>CompileAsCpp</CompileAs>" << std::endl;
//  file << "      <InlineFunctionExpansion>" << (debug ? "Disabled" : "AnySuitable") << "</InlineFunctionExpansion>" << std::endl;
//  file << "      <OpenMPSupport>" << (_wizard->useOpenMP() ? "true" : "false") << "</OpenMPSupport>" << std::endl;
//  file << "      <DebugInformationFormat>ProgramDatabase</DebugInformationFormat>" << std::endl;
//  file << "      <ProgramDatabaseFileName>" << binDirectory() << (_project->isExe() ? _name : name) << ".pdb</ProgramDatabaseFileName>" << std::endl;
//  file << "      <BasicRuntimeChecks>" << (debug ? "EnableFastChecks" : "Default") << "</BasicRuntimeChecks>" << std::endl;
//  file << "      <OmitFramePointers>" << (debug ? "false" : "true") << "</OmitFramePointers>" << std::endl;
//  file << "      <Optimization>" << (debug || _project->isOptimizationDisable() ? "Disabled" : "MaxSpeed") << "</Optimization>" << std::endl;
...
//  file << "      <PreprocessorDefinitions>" << (debug ? "_DEBUG" : "NDEBUG") <<";%(PreprocessorDefinitions)</PreprocessorDefinitions>" << std::endl;
...
//  {
//    file << "    <Lib>" << std::endl;
//    file << "      <AdditionalLibraryDirectories>" << libDirectory() << ";%(AdditionalLibraryDirectories)</AdditionalLibraryDirectories>" << std::endl;
...
//    file << ";%(AdditionalDependencies)</AdditionalDependencies>" << std::endl;
//    file << "      <SuppressStartupBanner>true</SuppressStartupBanner>" << std::endl;
//    file << "      <TargetMachine>Machine" << _wizard->machineName() << "</TargetMachine>" << std::endl;
//    file << "      <GenerateDebugInformation>" << (debug ? "true" : "false") << "</GenerateDebugInformation>" << std::endl;
//    file << "      <ProgramDatabaseFile>" << binDirectory() << (_project->isExe() ? _name : name) << ".pdb</ProgramDatabaseFile>" << std::endl;
//    file << "      <ImportLibrary>" << libDirectory() << name << ".lib</ImportLibrary>" << std::endl;
//    if (!_project->isConsole())
//    {
//      if (_project->isDll())
//        file << "      <LinkDLL>true</LinkDLL>" << std::endl;
//      else if (_project->useUnicode())
//        file << "    <EntryPointSymbol>wWinMainCRTStartup</EntryPointSymbol>" << std::endl;
//      file << "      <SubSystem>Windows</SubSystem>" << std::endl;
//      if ((_project->isDll()) && (!_project->moduleDefinitionFile().empty()))
//        file << "      <ModuleDefinitionFile>" << rootPath <<  _project->filePath(_project->moduleDefinitionFile()) << "</ModuleDefinitionFile>" << std::endl;
//    }
//    else
...
//          continue;
//
//        file << "    <ProjectReference Include=\"..\\" << deppf->name() << "\\" << deppf->_fileName << "\">" << std::endl;
//        file << "      <Project>{" << deppf->guid() << "}</Project>" << std::endl;
//        file << "      <ReferenceOutputAssembly>false</ReferenceOutputAssembly>" << std::endl;
