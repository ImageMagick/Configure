...
CommandLineInfo::CommandLineInfo(ConfigureOptions &options)
{
  _options=&options;
  showWizard=true;
}
...
void CommandLineInfo::ParseParam(const wchar_t* pszParam, BOOL bFlag, BOOL bLast)
{
  ...
  if (_wcsicmp(pszParam, L"arm64") == 0)
    ...
    _options->buildType=BuildType::Dynamic;
    ...
    _options->buildType=BuildType::Static;
