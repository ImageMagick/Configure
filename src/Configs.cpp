...
Config Configs::loadProjectConfig(const ConfigureOptions &options,const wstring &name,const wstring &directory)
{
...
void Configs::removeInvalidReferences(const ConfigureOptions &options,vector<Config> &configs)
{
  set<wstring> allNames;
  for (const auto& config : configs)
    allNames.insert(config.name());
  ...
  for (auto& config : configs)
  {
    ...
    {
      if (allNames.find(reference) == allNames.end() &&
         !filesystem::exists(options.rootDirectory + L"Artifacts\\include" + reference))
