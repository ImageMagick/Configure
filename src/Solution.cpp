...
std::map<ProjectType,std::wstring> Solution::createProjectTypeGuids()
{
  std::map<ProjectType,std::wstring> guids;

  guids[ProjectType::DynamicLibrary]=createGuid(L"Core");
  guids[ProjectType::StaticLibrary]=createGuid(L"Core");

  return(guids);
}

const std::wstring Solution::solutionName(const ConfigureOptions &options)
{
  std::wstring
    name(L"IM");
...
void Solution::write(const std::vector<Project> &projects,const ConfigureOptions &options)
{
  std::wstring solutionFileName = options.rootDirectory + solutionName(options);
  std::wofstream file(solutionFileName);
  if (!file)
    throwException(L"Failed to open file: " + solutionFileName);

  file << L"Microsoft Visual Studio Solution File, Format Version 12.00" << std::endl;
  writeVisualStudioVersion(file,options);
  writeProjects(file,projects);
  writeProjectFolder(file,L"Core");
  file << L"Global" << std::endl;
  file << L"  GlobalSection(SolutionConfigurationPlatforms) = preSolution" << std::endl;
  file << L"    Debug|" << options.architectureName() << " = Debug|" << options.architectureName() << std::endl;
  file << L"    Release|" << options.architectureName() << " = Release|" << options.architectureName() << std::endl;
  file << L"  EndGlobalSection" << std::endl;
  writeProjectsConfiguration(file,projects);
  writeProjectsNesting(file,projects);
  file << L"EndGlobal" << std::endl;

  file.close();
}

void Solution::writeProjectFolder(std::wofstream &file,const std::wstring &name)
{
  file << "Project(\"{2150E333-8FDC-42A3-9474-1A3956D46DE8}\") = \"" << name << "\", \"" << name << "\", \"{" << createGuid(name) << "}\"" << std::endl;
  file << "EndProject" << std::endl;
}

void Solution::writeProjects(std::wofstream& file,const std::vector<Project>& projects)
{
  for (const auto& project : projects)
  {
    file << "Project(\"{8BC9CEB8-8B4A-11D0-8D11-00A0C91BC942}\") = \"" << project.fullName() << "\", \"" << project.fileName() << "\", \"{" << project.guid() << "}\"" << std::endl;
    file << "EndProject" << std::endl;
  }
}

void Solution::writeProjectsConfiguration(std::wofstream& file,const std::vector<Project>& projects)
{  
  file << "  GlobalSection(ProjectConfigurationPlatforms) = postSolution" << std::endl;
  for (const auto& project : projects)
  {
    file << "    {" << project.guid() << L"}.Debug|x64.ActiveCfg = Debug|x64" << std::endl;
    file << "    {" << project.guid() << L"}.Debug|x64.Build.0 = Debug|x64" << std::endl;
    file << "    {" << project.guid() << L"}.Release|x64.ActiveCfg = Release|x64" << std::endl;
    file << "    {" << project.guid() << L"}.Release|x64.Build.0 = Release|x64" << std::endl;
  }
  file << "  EndGlobalSection" << std::endl;
}

void Solution::writeProjectsNesting(std::wofstream& file,const std::vector<Project>& projects)
{
  file << L"  GlobalSection(NestedProjects) = preSolution" << std::endl;
  auto guids=createProjectTypeGuids();
  for (const auto& project : projects)
  {
    file << L"    {" << project.guid() << L"} = {" << guids.at(project.type()) << L"}" << std::endl;
  }
  file << L"  EndGlobalSection" << std::endl;
}

void Solution::writeVisualStudioVersion(std::wofstream& file,const ConfigureOptions &options)
{
  switch(options.visualStudioVersion)
  {
  case VisualStudioVersion::VS2022:
    file << "# Visual Studio 17" << std::endl;
    file << "VisualStudioVersion = 17.0.31903.59" << std::endl;
    break;
  case VisualStudioVersion::VS2019:
    file << "# Visual Studio 16" << std::endl;
    file << "VisualStudioVersion = 16.0.28701.123" << std::endl;
    break;
  case VisualStudioVersion::VS2017:
    file << "# Visual Studio 15" << std::endl;
    file << "VisualStudioVersion = 15.0.26124.0" << std::endl;
    break;
  }
