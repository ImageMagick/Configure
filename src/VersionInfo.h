...
public:
  ...
  static optional<VersionInfo> load(const ConfigureOptions& options);

  void write(const ConfigureOptions& options) const;

  void write(wstring inputFile,wstring outputFile) const;

private:
  VersionInfo(const ConfigureOptions& options);
