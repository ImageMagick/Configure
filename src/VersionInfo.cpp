...
VersionInfo::VersionInfo(const ConfigureOptions& options)
  : _options(options)
{
}
...
optional<VersionInfo> VersionInfo::load(const ConfigureOptions& options)
{
...
wstring VersionInfo::replaceVariable(const wstring &line,const size_t start) const
{
  ...
  if (skipableKeywords.end() != find(skipableKeywords.begin(), skipableKeywords.end(), keyword))
    return(L"");
...
void VersionInfo::write(const ConfigureOptions& options) const
{
  wstring versionFile=L"ImageMagick\\" + options.magickCoreName() + L"\\version.h";

  write(L"Build\\package.version.h.in",L"Build\\package.version.h");
  write(L"ImageMagick\\" + options.magickCoreName() + L"\\version.h.in",versionFile);
