...
ConfigureOptions::ConfigureOptions(const wstring &rootDirectory)
...
const wstring ConfigureOptions::architectureName() const
{
  switch (architecture)
  {
...
const wstring ConfigureOptions::buildTypeName() const
{
  switch (buildType)
  {
...
const wstring ConfigureOptions::channelMaskDepth() const
{
  if (version == ImageMagickVersion::ImageMagick6)
    return(L"");
...
const wstring ConfigureOptions::platform() const
{
  switch (architecture)
  {
...
const wstring ConfigureOptions::projectsDirectory() const
{
  return(L"ProjectFiles\\" + architectureName() + L"\\");
}

wstring ConfigureOptions::getEnvironmentVariable(const wchar_t *name)
{
  wchar_t
    *buffer;
