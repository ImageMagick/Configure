...
vector<Project> Projects::create(const ConfigureOptions &options,vector<Config> &configs)
{
  vector<Project>
    projects;
  ...
  removeProjectsWithInvalidReferences(options,projects);

  return(projects);
}

void Projects::createCoderProjects(const ConfigureOptions &options,vector<Config> &configs,vector<Project> &projects)
{
...
void Projects::createDemoProjects(const ConfigureOptions &options,vector<Config> &configs,vector<Project> &projects)
{
...
void Projects::createFuzzProjects(const ConfigureOptions &options,vector<Config> &configs,vector<Project> &projects)
{
...
void Projects::createUtilitiesProjects(const ConfigureOptions &options,vector<Config> &configs,vector<Project> &projects)
{
...
void Projects::removeProjectsWithInvalidReferences(const ConfigureOptions &options,vector<Project> &projects)
{
  vector<Project> validProjects;
  validProjects.reserve(projects.size());
  ...
  for(auto& project : projects)
  {
    ...
    {
      auto referencedProject=find_if(projects.begin(),projects.end(),[&](const auto &p) { return (p.isLibrary() && p.name() == reference); });
      if (referencedProject == projects.end())
      {
        if (filesystem::exists(options.rootDirectory + L"Artifacts\\include" + reference))
          continue;
    ...
    if (isValid)
      validProjects.push_back(project);
  }

  projects=std::move(validProjects);
}
